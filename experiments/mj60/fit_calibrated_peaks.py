...
def peak_2615():
    ...
    hist, bins, var = pgh.get_hist(df['e_real'], range=(2590,2640), dx=0.5)
...
def peak_1460():
    ...
    hist, bins, var = pgh.get_hist(df['e_real'], range=(1445,1475), dx=0.5)
    pgh.plot_hist(hist, bins, var=hist, label="data")
...
def peak_969():
    ...
    hist, bins, var = pgh.get_hist(df['e_real'], range=(945,990), dx=0.5)
    pgh.plot_hist(hist, bins, var=hist, label="data")
    pars, cov = pga.fit_hist(radford_peak, hist, bins, var=hist, guess=[968.4, 2.1, 0.003, 0.9, 2, 7, 900])
    #pgu.print_fit_results(pars, cov, radford_peak)
    print(pars[6])
    pgu.plot_func(radford_peak, pars, label="chi2 fit", color='red')
...
def peak_911():
    ...
    hist, bins, var = pgh.get_hist(df['e_real'], range=(890,930), dx=0.5)
    pgh.plot_hist(hist, bins, var=hist, label="data")
...
def peak_583():
    ...
    hist, bins, var = pgh.get_hist(df['e_real'], range=(560,600), dx=0.5)
    pgh.plot_hist(hist, bins, var=hist, label="data")
...
def peak_511():
    ...
    hist, bins, var = pgh.get_hist(df['e_real'], range=(495,525), dx=0.5)
    pgh.plot_hist(hist, bins, var=hist, label="data")
