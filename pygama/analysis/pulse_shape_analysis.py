...
def get_avse_cut(e_cal, current, plotFigure=None):
    ...
    if plotFigure is not None:
        ...
        h_dep,bins  = np.histogram(a_over_e[dep_idxs] , bins=np.linspace(-12*ae_std,8*ae_std,100))
        ...
        ax_ae.plot(bin_centers,h_bgs ,  ls="steps-mid", color = "b", label = "DEP (BG subtracted)")
        ax_ae.plot(bin_centers, h_bgs_sep, ls="steps-mid", color = "g", label = "SEP (BG subtracted)")
