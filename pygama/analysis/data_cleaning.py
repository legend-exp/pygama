...
def gaussian_cut(data, cut_sigma=3,plotAxis=None):
    ...
    if plotAxis is not None:
        ...
        plotAxis.axvline(cut_lo, color = "r", label = "+/- {} sigma".format(cut_sigma) )
...
def find_pulser_properties(df, energy="trap_max"):
    ...
    for e in peak_energies:
        ...
        tsl = time_since_last[(time_since_last>=0)&(time_since_last<np.percentile(time_since_last, 99.9))]
...
def tag_pulsers(df, chan_info, window=250):
    ...
    e_cut = (df[energy_name] < pulser_energy+peak_e_err)&(df[energy_name] > pulser_energy-peak_e_err)
    df_pulser = df[e_cut]



    time_since_last = np.zeros(len(df_pulser))
    time_since_last[1:] = df_pulser.timestamp.values[1:] - df_pulser.timestamp.values[:-1]

    # plt.figure()
    # plt.hist(time_since_last, bins=1000)
    # plt.show()

    mode_idxs = (time_since_last>period-window)&(time_since_last<period+window)
