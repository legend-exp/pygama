...
def daq_to_raw(t0_file, run, prefix="t1", suffix="", chan_list=None, n_max=np.inf,
    ...
    if config["daq"] == "ORCA":
        ...
        process_orca2(t0_file, t1_file, n_max, decoders, config, verbose, run)
...
def process_orca2(t0_file, t1_file, n_max, decoders, config, verbose, run=None):
    """
    convert ORCA DAQ data to pygama "raw" lh5
    """
    table_buffer_size = 1023
