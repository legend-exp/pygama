...
Primary processing routines and digitizer data.

NOTE: file name string formatting is still under discussion, expect this to change.

* `daq_to_raw` (formerly "Tier 0"): convert data from various DAQ systems to the LEGEND format specification, `tier1_[run_num].lh5`.

* `raw_to_dsp` (Formerly "Tier 1"): run digital signal processing routines (typically specified by a JSON-formatted config file) on the raw data, `tier2_[run_num].lh5`
