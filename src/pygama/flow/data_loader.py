    ...
    def gen_entry_list(self, tcm_level=None, tcm_table=None, mode='only', save_output_columns=False, in_mem=True, f_output=None): #TODO: mode
        ...
        if tcm_level is None:
            ...
            for file in self.file_list:
                ...
                tcm_lgdo[self.tcms[tcm_level]["tcm_cols"]["child_idx"]] = Array(nda=explode_cl(tcm_lgdo['cumulative_length'].nda))
                        ...
                        for tier in self.tiers[level]:
                            ...
                            if tier in col_tiers[file]["tables"].keys():
                                if tb in col_tiers[file]["tables"][tier]:
                                    ...
                                    tier_table, _ = sto.read_object(table_name, tier_path, field_mask=cut_cols[level], idx=idx_mask.tolist())
                                    if tb_table is None:
                                        tb_table = tier_table 
                                    else:
                        ...
                        for tier in self.tiers[level]:
                            if tb in col_tiers["tables"][tier]:
                                ...
                                if path.os.exists(tier_path):
                                    table_name = self.get_table_name(tier, tb)
                                    tier_table, _ = sto.read_object(table_name, file_path, idx=idx_mask, field_mask=field_mask)
                                    for col in tier_table.keys():
...
if __name__=='__main__':
    ...
    print("Full FileDB: ")
    dl = DataLoader(config="../../../../loader_config.json", fileDB_config="fileDB_cfg.json", fileDB="fileDB.lh5")
    dl.show_fileDB()
    print()
    ...
    dl.show_file_list(['period', 'run', 'type', 'raw_tables', 'dsp_tables', 'tcm_tables', 'file_status'])
    print()
