...
class OrcaHeaderDecoder(OrcaDecoder):
    ...
    def decode_packet(self, packet: OrcaPacket, packet_id: int,
                      rbl: RawBufferLibrary = None) -> bool:
        ...
        if rbl is not None:
            if len(rbl) != 1 or not isinstance(rbl[0].lgdo, lgdo.Scalar):
                raise ValueError(f"invalid RawBufferLibrary object: len(rbl) = {len(rbl)}, type = {type(rbl[0].lgdo)}")
            else:
