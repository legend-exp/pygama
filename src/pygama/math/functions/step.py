...
@nb.njit(**kwd)
def nb_unnorm_step_pdf(x,  mu, sigma, hstep):
    """
...
@nb.njit(**kwd)
def nb_step_pdf(x,  mu, sigma, hstep, lower_range=np.inf , upper_range=np.inf):
    """
    ...
    'out of the box' functions.
    """
    step_f = nb_unnorm_step_pdf(x,  mu, sigma, hstep)
...
@nb.njit(**kwd)
def nb_step_cdf(x,mu,sigma, hstep, lower_range=np.inf , upper_range=np.inf):
    """
    CDF for step function w/args mu, sigma, hstep
    As a Numba JIT function, it runs slightly faster than
    'out of the box' functions.
    """
    cdf = nb_step_int(x,mu,sigma,hstep)
    if lower_range ==np.inf and upper_range ==np.inf:
        integral = nb_step_int(np.array([np.nanmin(x), np.nanmax(x)]), mu, sigma, hstep)
    else:
        integral = nb_step_int(np.array([lower_range, upper_range]), mu, sigma, hstep)
...
@nb.njit(**kwd)
def nb_step_int(x,mu,sigma, hstep):
    """
    ...
    part2 = - np.sqrt(2/np.pi)*hstep*sigma*nb_gauss(x,mu,sigma)
    return  part1-part2
