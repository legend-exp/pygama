...
def nb_step_int(x: float, mu: float, sigma: float, hstep: float) -> np.ndarray:
    ...
    if sigma == 0:
        ...
        z = (x-mu)/sigma
        y = sigma*(z + hstep*(z*erf(z/np.sqrt(2)) + np.sqrt(2/np.pi)*np.exp(-z**2/2)))
    return y
...
class step_gen(pygama_continuous):
    ...
    def _cdf(self, x: np.ndarray, x_lo: float, x_hi: float, mu: float, sigma: float, hstep: float) -> np.ndarray:
        x.flags.writeable = True
        return nb_step_cdf(x, x_lo[0], x_hi[0], mu[0], sigma[0], hstep[0])

    def get_pdf(self, x: np.ndarray, x_lo: float, x_hi: float, mu: float, sigma: float, hstep: float) -> np.ndarray:
        return nb_step_pdf(x, x_lo, x_hi, mu, sigma, hstep)
    def get_cdf(self, x: np.ndarray, x_lo: float, x_hi: float, mu: float, sigma: float, hstep: float) -> np.ndarray:
    ...
    def pdf_norm(self, x: np.ndarray, x_lo: float, x_hi: float, mu: float, sigma: float, hstep: float) -> np.ndarray: 
        return nb_step_pdf(x, x_lo, x_hi, mu, sigma, hstep)
    def cdf_norm(self, x: np.ndarray, x_lo: float, x_hi: float, mu: float, sigma: float, hstep: float) -> np.ndarray: 
        return nb_step_cdf(x, x_lo, x_hi, mu, sigma, hstep)
