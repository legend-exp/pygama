...
def evaluate_expression(f_evt:str,f_hit:str, f_dsp: str, chns: list, mode: str, expr: str, para: dict = None, defv = np.nan, nrows: int = None) -> np.ndarray:
     """
     ...
     n = nrows if nrows is not None else store.LH5Store().read_n_rows(chns[0]+"/dsp/",f_dsp)
     ...
     var_ph = {}
     if os.path.exists(f_evt):var_ph = store.load_nda(f_evt,[e.split('/')[-1] for e in store.ls(f_evt) if e.split('/')[-1] in exprl])
     if para: var_ph = var_ph | para
     
     if mode == "func":
        # evaluate expression
        func, params = expr.split('(')
        params = [f_hit,f_dsp,chns]+[num_and_pars(e,var_ph) for e in params[:-1].split(',')]
        ...
        for ch in chns:
            ...
            var = store.load_nda(f_hit,[e.split('/')[-1] for e in store.ls(f_hit,ch+"/hit/") if e.split('/')[-1] in exprl],ch+"/hit/")
            dsp_dic = store.load_nda(f_dsp,[e.split('/')[-1] for e in store.ls(f_dsp,ch+"/dsp/") if e.split('/')[-1] in exprl],ch+"/dsp/")
            var= var |dsp_dic | var_ph
        
            # evaluate expression
            res = eval(expr,var)
            if not isinstance(res, np.ndarray): res = np.full(len(out),res,dtype=type(res))

            # append to out according to mode
            if  len(ops)>0: limarr = eval("".join(["res",ops[0],"lim"]),{"res":res,"lim":float(mode.split(ops[0])[-1])})
            else: limarr = np.ones(len(out)).astype(bool)
            if "first" in mode:
                outt = np.full(len(out),np.inf)
...
def build_evt(
    ...
    for k, v in tbl_cfg['operations'].items(): 
         ...
         if 'channels' not in v.keys():
            ...
            res,chs = evaluate_expression(f_evt,f_hit,f_dsp,chns_e,v['mode'],v['expression'],pars,defaultv,nrows)
            lstore.write_object(
                    obj=Array(res),
                    name= k,
                    lh5_file=f_evt,
            ...
            if "get_ch" in v.keys() and ("first" in v['mode'] or "last" in v['mode']) and v["get_ch"]:
                lstore.write_object(
                    obj=Array(chs),
                    name= k+"_id",
                    lh5_file=f_evt,
