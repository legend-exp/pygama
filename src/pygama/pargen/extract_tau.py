...
def run_tau(raw_file:list[str], config:dict, channel:str, n_events:int=30000, threshold:int=3000)->lgdo.Table:
    ...
    waveforms = sto.read_object(f'{channel}/raw/waveform', raw_file,idx=cuts, n_rows = n_events)[0]
    baseline = sto.read_object(f'{channel}/raw/baseline', raw_file,idx=cuts, n_rows = n_events)[0]
