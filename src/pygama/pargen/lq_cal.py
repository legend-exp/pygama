...
def get_lq_hist(
    ...
    if sidebands:
        ...
        peak_window = (df[cal_energy_param] < peak + 3) & (df[cal_energy_param] > peak - 3)
        sideband_window = (df[cal_energy_param] < peak + 18) & (df[cal_energy_param] > peak + 12)
        
        fit_range = get_fit_range(df[lq_param][peak_window])

        sideband_hist, bins, _ = pgh.get_hist(df[lq_param][sideband_window], bins = 100, range = fit_range)
        dep_hist, _, _ = pgh.get_hist(df[lq_param][peak_window], bins = 100, range = fit_range)
        ...
        peak_window = (df[cal_energy_param] < peak + 2.5) & (df[cal_energy_param] > peak - 2.5)
        fit_range = get_fit_range(df[lq_param][peak_window])
        dep_hist, bins, var = pgh.get_hist(df[lq_param][peak_window], bins = 100, range = fit_range)
...
class cal_lq:
    ...
    def drift_time_correction(self, df: pd.DataFrame(), lq_param, cal_energy_param: str, display: int = 0):
        ...
        try:
            ...
            lq_mask = (df[lq_param] < (2*sigma + mean)) & (df[lq_param] > (mean - 2*sigma))
            dep_mask = (df[cal_energy_param] < 1595) & (df[cal_energy_param] > 1590)     

            ids = np.isnan(df[lq_param]) | np.isnan(df['dt_eff'])
            result = linregress(df['dt_eff'][~ids & dep_mask & lq_mask], df[lq_param][~ids & dep_mask & lq_mask], alternative = 'greater')
            self.dt_fit_pars = result

            df['LQ_Classifier'] = df[lq_param] - df['dt_eff']*self.dt_fit_pars[0] - self.dt_fit_pars[1]
    ...
    def get_cut_lq_dep(self, df: pd.DataFrame(), lq_param: str, cal_energy_param: str):
        ...
        try:
            pars, errs, hist, bins = binned_lq_fit(df, 'LQ_Classifier', cal_energy_param, peak = 1592.5)
            cut_val = 3*pars[1]
    ...
    def calibrate(self, df, initial_lq_param):
        ...
        self.drift_time_correction(df, lq_param = 'LQ_Timecorr', cal_energy_param = self.cal_energy_param)
        log.info('Finished LQ Drift Time Correction')
        
        self.get_cut_lq_dep(df, lq_param = 'LQ_Classifier', cal_energy_param = self.cal_energy_param)
        ...
        for i, peak in enumerate(peaks_of_interest):
            try:
                ...
                log.error(
                    f"LQ Survival fraction determination failed for {peak} peak"
                )
...
def plot_drift_time_correction(
    ...
    try:
        ...
        initial_df = data[(data[lq_class.cal_energy_param] > dep_range[0]) & (data[lq_class.cal_energy_param] < dep_range[1])]
        max_dt = 1500
        max_lq = 2.5

        plt.hist2d(initial_df['dt_eff'], 
                   initial_df[lq_param], 
                   bins = 100, 
                   range = ((0, max_dt), (0, max_lq)), norm = mcolors.LogNorm()
                  )
...
def plot_lq_cut_fit(
    ...
    try:
        ...
        ls = np.sum(hist)
        dx = np.diff(bins)
        ax1.plot(xs, pgf.gauss_pdf(xs, fit_pars[0], fit_pars[1], ls)*dx, label = 'Gaussian Fit')
        ...
        bin_centers = (bins[:-1] + bins[1:]) / 2
        reses = (hist - (pgf.gauss_pdf(bin_centers, fit_pars[0], fit_pars[1], ls)*dx)) / (pgf.gauss_pdf(bin_centers, fit_pars[0], fit_pars[1], ls)*dx)
