...
class CalAoE:
    ...
    def MCDrift(
        ...
        try:
            ...
            CC_selection = (
                (CC_events[self.cal_energy_param] >= 1000) &
            ...
            dt_vals  = CC_events[self.dt_param].values/1000.0 #Compute alpha in us for more stability
            ...
            in_y  = (mcd_data[:, 1] >= 0) & (mcd_data[:, 1] <= np.quantile(mcd_data[:, 1], 0.999))
            ...
            if abs(principal[0]) > abs(principal[1]):
                log.warning(
                    ...
                    "switching to minor eigenvector", abs(principal[0]), abs(principal[1])
                )
                principal = eigvecs[:, -2]
                
            if abs(principal[1]) < 1e-10:
                 raise ValueError("Eigenvector has near-zero dt component, alpha set to 0")
