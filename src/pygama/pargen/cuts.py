...
def cut_dict_to_hit_dict(cut_dict):
    ...
    for param in cut_dict:
        ...
        out_dict[f'{param}_cut'] = {"expression":f'@a<{param}<@b', 
                                    'parameters': {
    ...
    out_dict['Quality_cuts'] = {"expression":quality_cut_exp, 
                                    'parameters': {}
                                    }
    return out_dict
...
def find_pulser_properties(df, energy="daqenergy"):
    ...
    hist,bins, var = pgh.get_hist(df['daqenergy'], dx=1, 
                              range=(100,np.nanmax(df['daqenergy'])))
    if np.any(var == 0):
