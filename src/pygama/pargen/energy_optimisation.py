...
def run_optimisation_multiprocessed(file,opt_config,dsp_config, cuts, lh5_path, fom=None, db_dict=None, processes=5, n_events=8000, **fom_kwargs):
    """
    ...
    sto=lh5.LH5Store()
    waveforms = sto.read_object(f'{lh5_path}/waveform', file,idx=cuts, n_rows = n_events)[0]
    baseline = sto.read_object(f'{lh5_path}/baseline', file,idx=cuts, n_rows = n_events)[0]
    tb_data = lh5.Table(col_dict = { 'waveform' : waveforms, 'baseline':baseline } )
...
def set_values(par_values):
    ...
    """
    string_values=np.arange(par_values['start'],par_values['end'],par_values['spacing'])
    try:
...
def unbinned_energy_fit(energy, func, gof_func, gof_range, fit_range= (np.inf,np.inf),guess=None,tol=None,verbose=False, display=0):
...
def get_peak_fwhm_with_dt_corr(Energies, alpha,dt, func, gof_func, peak, kev_width, guess=None, kev=False, display=0):
    ...
    try:   
        if display >0:
            energy_pars, energy_err, cov, chisqr = unbinned_energy_fit(ct_energy[win_idxs], func, gof_func,
                                                                        gof_range,fit_range, tol=tol ,guess=guess, 
            ...
            energy_pars, energy_err, cov, chisqr = unbinned_energy_fit(ct_energy[win_idxs], func, gof_func,
                                                                        gof_range,fit_range ,guess=guess, tol=tol)
    ...
    return fwhm, fwhm_o_max, fwhm_err,fwhm_o_max_err, chisqr, energy_pars[0], energy_err[0], energy_pars
...
def fom_FWHM_with_dt_corr_fit(tb_in, kwarg_dict, ctc_parameter, idxs = None, display=0):
    ...
    best_fwhm = np.inf
    for alpha in alphas:
        _, fwhm_o_max,_,fwhm_o_max_err,_,_,_,fit_pars = get_peak_fwhm_with_dt_corr(Energies, alpha,dt, func, 
                                                                        gof_func, peak, kev_width, guess=guess)
    ...
    if np.isnan(fit_vals).all():
        ...
        final_fwhm, _, final_err,_, csqr, n_sig, n_sig_err,_ = get_peak_fwhm_with_dt_corr(Energies, alpha,dt, func, 
                                                                                        gof_func,peak,kev_width, 
                                                                                        guess = best_fit, kev=True, display=display)
        if np.isnan(final_fwhm) or np.isnan(final_err):
            final_fwhm, _, final_err,_, csqr, n_sig, n_sig_err,_ = get_peak_fwhm_with_dt_corr(Energies, alpha,dt, func, 
                                                                                        gof_func,peak,kev_width, 
                                                                                        kev=True, display=display)
        if np.isnan(final_fwhm) or np.isnan(final_err):    
...
def event_selection(raw_file, lh5_path, dsp_config, db_dict, peaks_keV, peak_idx, kev_width, n_events=10000,threshold=3000):
    """
    ...
    waveforms = sto.read_object(f'{lh5_path}/waveform', raw_file, idx=e_idxs, n_rows=int(2.5*n_events))[0]
    baseline = sto.read_object(f'{lh5_path}/baseline', raw_file, idx=e_idxs, n_rows=int(2.5*n_events))[0]
    ...
    hist, bins, var = pgh.get_hist(energy, range=(int(threshold), int(np.nanmax(energy))), dx=1)
...
def fom_FWHM(tb_in, kwarg_dict, ctc_parameter, alpha,  idxs = None, display=0):
    ...
    # Return fwhm of optimal alpha in kev with error
    try:
        final_fwhm, _, final_err,_, csqr, n_sig, n_sig_err,_ = get_peak_fwhm_with_dt_corr(Energies, alpha,dt, 
                                                                                        func, cs_func,
                                                                                        peak,kev_width ,
                                                                                        kev=True, display=display)
    except:
...
def new_fom(data, kwarg_dict):
    ...
    out_dict = fom_FWHM_with_dt_corr_fit(data, peak_dicts[-1], ctc_param, idxs = idx_list[-1], display=0)
    ...
    for i,peak in enumerate(peaks[:-1]):
        out_peak_dict = fom_FWHM(data,peak_dicts[i], ctc_param, alpha, idxs =idx_list[i], display=0)
    ...
    qbb, qbb_err, fit_pars = interpolate_energy(np.array(peaks), np.array(fwhms), np.array(fwhm_errs), 2039)

    log.info(f'Qbb fwhm is {qbb} keV +- {qbb_err}')
    
    return {'y_val':qbb, 'qbb_fwhm': qbb, 'qbb_fwhm_err': qbb_err, 
            'alpha': alpha, 'peaks': peaks.tolist(), 'fwhms':fwhms, 'fwhm_errs':fwhm_errs,
            "n_events": n_sig, 'n_sig_err':n_sig_err}

OptimiserDimension = namedtuple('OptimiserDimension', 'name parameter min_val max_val unit')
...
class BayesianOptimizer():
    ...
    def _get_expected_improvement(self, x_new):
        ...
        exp_imp = (mean_y_new - min_mean_y-self.eta_param) * norm.cdf(z) + sigma_y_new * norm.pdf(z)
        return exp_imp
    ...
    def _get_next_probable_point(self):
            ...
            rands = (np.random.uniform(np.array([dim.min_val for dim in self.dims]),
                                       np.array([dim.max_val for dim in self.dims]),
                                       (self.batch_size,self.get_n_dimensions())))
            for x_start in rands:
                response = minimize(fun=self._acquisition_function, x0=x_start, 
                                    bounds = [(dim.min_val,dim.max_val)for dim in self.dims] ,method='L-BFGS-B')
            ...
            if x_optimal in self.x_init and self.iters==5:
                x_optimal += np.random.uniform(np.array([0.1 for dim in self.dims]),np.array([1 for dim in self.dims]) , (1,len(self.dims)))
    ...
    def update(self, results):
        ...
        if self.current_iter == 1:
            ...
            self.distances_.append(np.linalg.norm(np.array(self.prev_x) - np.array(self.current_x)))
            self.prev_x = self.current_x

        self.best_samples_ = self.best_samples_.append({"y": self.y_min, "ei": self.optimal_ei},ignore_index=True)
...
def run_optimisation(tb_data, dsp_config, fom_function, optimisers, fom_kwargs=None,db_dict=None, nan_val=10, n_iter=10):
    ...
    for j in range(n_iter):
        ...
        tb_out = opt.run_one_dsp(tb_data,
                        dsp_config,
                        db_dict=db_dict)
