...
def test_get_multi_local_extrema_LtoR(compare_numba_vs_python):
    wf = np.array([0,0,1,2,3,4,5,4,3,4,5,6,7,8,9,10,9,8,7,6,5,4,3,4,5,4,3,2,1,0,0])
    max_out = np.zeros(3)
    max_out[:]= np.nan
    min_out = np.zeros(3)
    min_out[:]=np.nan
...
# asymmetric delta test (result should be search direction dependend)
...
#Absolute threshold test with aggresive both direction search
...
def test_get_multi_local_extrema_return_on_nan(compare_numba_vs_python):
    ...
    wf[4] = np.nan
    assert np.isnan(compare_numba_vs_python(
        get_multi_local_extrema,
        wf,                         
        3,
        1,                          
