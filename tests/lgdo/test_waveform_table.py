...
def test_init():
    wft = WaveformTable()  # size = 1024 and wf_len = 100
    ...
    wft = WaveformTable(values=lgdo.ArrayOfEqualSizedArrays(shape=(10, 1000), fill_val=69))
    assert (wft.t0.nda == np.zeros(10)).all()
    assert (wft.dt.nda == np.full(10, fill_value=1)).all()
    assert isinstance(wft.values, lgdo.ArrayOfEqualSizedArrays)
    ...
    # wft = WaveformTable(values=lgdo.VectorOfVectors(shape_guess=(10, 1000)))
    # assert (wft.t0.nda == np.zeros(10)).all()
    # assert (wft.dt.nda == np.full(10, fill_value=1)).all()
    # assert isinstance(wft.values, lgdo.VectorOfVectors)
    # assert (wft.values.nda == np.full(shape=(10, 1000), fill_value=69)).all()

    wft = WaveformTable(t0=[1, 1, 1], dt=[2, 2, 2], values=lgdo.ArrayOfEqualSizedArrays(shape=(3, 1000)))
