...
def test_datatype_name():
    ...
    assert array.datatype_name() == 'fixed_size_array'


def test_form_datatype():
    array = lgdo.FixedSizeArray()
    assert array.form_datatype() == 'fixed_size_array<0>{real}'
...
def test_init():
    ...
    assert array.attrs == attrs | {'datatype': 'fixed_size_array<1>{real}'}
