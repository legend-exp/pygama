...
def test_cross_talk_corrected_energy_awkard_slow():
    ...
    with pytest.raises(ValueError):
        cross_talk.cross_talk_corrected_energy_awkard_slow(energies_test[energies_test<600],rawid_test,matrix,True)

    ## checks on the matrix
    # first check if the matrix has empty elements an exception is raised if allow_non_existing is false
    matrix_not_full={1:{1:1.00,  3:0.02, 4:0.00},
                    2:{2:1.00, 3:0},
                    3:{1:0.02, 2:0.00, 3:1.00, 4:0.01},
    ...
    assert np.allclose(energy_corr[2].to_numpy(),np.array([532.,3005.,110.]))


    ## test with a threshold of 1 MeV

    energy_corr_threshold =cross_talk.cross_talk_corrected_energy_awkard_slow(energies_test,rawid_test,matrix,True,threshold=1000)   
