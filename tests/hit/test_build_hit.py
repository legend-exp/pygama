...
def test_basics(dsp_test_file, tmpdir_factory):
    outfile = f"{tmpdir_factory}/LDQTA_r117_20200110T105115Z_cal_geds_hit.lh5"
...
def test_lh5_table_configs(dsp_test_file, tmpdir_factory):
    outfile = f"{tmpdir_factory}/LDQTA_r117_20200110T105115Z_cal_geds_hit.lh5"
...
def test_outputs_specification(dsp_test_file, tmpdir_factory):
    outfile = f"{tmpdir_factory}/LDQTA_r117_20200110T105115Z_cal_geds_hit.lh5"
...
def test_aggregation_outputs(dsp_test_file, tmpdir_factory):
    outfile = f"{tmpdir_factory}/LDQTA_r117_20200110T105115Z_cal_geds_hit.lh5"
...
def test_build_hit_spms_basic(dsp_test_file_spm, tmpdir_factory):
    out_file = f"{tmpdir_factory}/L200-comm-20211130-phy-spms_hit.lh5"
    build_hit(
        dsp_test_file_spm,
        outfile=out_file,
...
def test_build_hit_spms_multiconfig(dsp_test_file_spm, tmpdir_factory):
    out_file = f"{tmpdir_factory}/L200-comm-20211130-phy-spms_hit.lh5"

    # append the tmpdir_factory to the start of paths in the spms-hit-multi-config.json
    ...
    with open(f"{tmpdir_factory}/spms-hit-multi-config.json", "w") as file:
        file.write(newdict)

    build_hit(
        dsp_test_file_spm,
        outfile=out_file,
        lh5_tables_config=f"{tmpdir_factory}/spms-hit-multi-config.json",
        wo_mode="overwrite",
    )
    assert lh5.ls(out_file) == ["ch0", "ch1", "ch2"]
    assert lh5.ls(out_file, "ch0/") == ["ch0/hit"]
...
def test_build_hit_spms_calc(dsp_test_file_spm, tmpdir_factory):
    out_file = f"{tmpdir_factory}/L200-comm-20211130-phy-spms_hit.lh5"
...
def test_vov_input(lgnd_test_data, tmpdir_factory):
