...
def test_filedb_basics(test_filedb):
    ...
    assert list(db.df.keys()) == [
        'exp', 'period', 'run', 'timestamp', 'type', 'raw_file', 'dsp_file',
        'hit_file', 'tcm_file', 'evt_file', 'raw_size', 'dsp_size', 'hit_size',
        'tcm_size', 'evt_size', 'file_status'
    ]

    assert db.df.values.tolist() == [
        ['l60', 'p01', 'r014', '20220716T105236Z', 'cal',
         ...
         2141080, 1371544, 0, 15728, 0, int('0b11010', 2)],
        ['l60', 'p01', 'r014', '20220716T104550Z', 'cal',
         ...
         2141080, 2060344, 0, 15728, 0, int('0b11010', 2)]
    ]
...
def test_get_tables_columns(test_filedb):
    ...
    assert list(db.df.keys()) == [
        ...
        'hit_file', 'tcm_file', 'evt_file', 'raw_size', 'dsp_size', 'hit_size',
        'tcm_size', 'evt_size', 'file_status',
        'raw_tables', 'raw_col_idx', 'dsp_tables', 'dsp_col_idx',
        'hit_tables', 'hit_col_idx', 'tcm_tables', 'tcm_col_idx',
        'evt_tables', 'evt_col_idx'
    ]

    assert db.columns == [
        ['baseline', 'card', 'ch_orca', 'channel', 'crate', 'daqenergy',
         'deadtime', 'dr_maxticks', 'dr_start_pps', 'dr_start_ticks',
         'dr_stop_pps', 'dr_stop_ticks', 'eventnumber', 'fcid', 'numtraces',
         'packet_id', 'runtime', 'timestamp', 'to_abs_mu_usec',
         'to_dt_mu_usec', 'to_master_sec', 'to_mu_sec', 'to_mu_usec',
         'to_start_sec', 'to_start_usec', 'tracelist', 'ts_maxticks', 'ts_pps',
         'ts_ticks', 'waveform'],
        ['bl_intercept', 'bl_mean', 'bl_slope', 'bl_std', 'tail_slope',
         'tail_std', 'wf_blsub'],
