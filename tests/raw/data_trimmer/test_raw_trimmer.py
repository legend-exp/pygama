...
# check that packet indexes match in verification test
def test_raw_trimmer_waveform_lengths(lgnd_test_data):

    # Set up I/O files, including config
    daq_file = lgnd_test_data.get_path("fcio/L200-comm-20211130-phy-spms.fcio")
    raw_file = lgnd_test_data.get_path("fcio/L200-comm-20211130-phy-spms.lh5")
    trimmed_file = raw_file.replace(
        "L200-comm-20211130-phy-spms.lh5", "L200-comm-20211130-phy-spms_trim.lh5"
    )
    ...
    raw_trimmer(
        ...
        overwrite=True,
        trim_config=dsp_config,
        trim_file_name=trimmed_file,
    )
...
def test_raw_trimmer_file_size_decrease(lgnd_test_data):
    # Set up I/O files, including config
    daq_file = lgnd_test_data.get_path("orca/sis3316/coherent-run1141-bkg.orca")
    raw_file = lgnd_test_data.get_path("orca/sis3316/coherent-run1141-bkg.lh5")
    dsp_config = f"{config_dir}/data_trimmer_config.json"
    trimmed_file = raw_file.replace(
        "coherent-run1141-bkg.lh5", "coherent-run1141-bkg_trim.lh5"
    )
...
def test_raw_trimmer_separate_name_tables(lgnd_test_data):

    # Set up I/O files, including config
    daq_file = lgnd_test_data.get_path("fcio/L200-comm-20211130-phy-spms.fcio")
    raw_file = lgnd_test_data.get_path("fcio/L200-comm-fake-geds-and-spms.lh5")
    trimmed_file = raw_file.replace(
        "L200-comm-fake-geds-and-spms.lh5", "L200-comm-fake-geds-and-spms_trim.lh5"
    )
...
def test_raw_geds_no_trim_spms(lgnd_test_data):
    # Set up I/O files, including config
    daq_file = lgnd_test_data.get_path("fcio/L200-comm-20211130-phy-spms.fcio")
    raw_file = lgnd_test_data.get_path("fcio/L200-comm-test-pass.lh5")
    trimmed_file = raw_file.replace(
        "L200-comm-test-pass.lh5", "L200-comm-test-pass_trim.lh5"
    )
