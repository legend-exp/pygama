...
def test_only_two_distributions():
    with pytest.raises(ValueError) as exc_info:
        sum_dists([(gaussian, [0, 1])], [0], "fracs")
...
def test_two_distributions_no_par_array():
    with pytest.raises(ValueError) as exc_info:
        sum_dists([(gaussian, [0, 1]), (gaussian, [], [])], [0], "fracs")
...
def test_pygama_continuous_check():
    with pytest.raises(ValueError) as exc_info:
        sum_dists([(gaussian, [0, 1]), (norm, [1, 0])], [0], "fracs")
...
def test_fracs_flag_with_wrong_args():
    with pytest.raises(ValueError) as exc_info:
        sum_dists([(gaussian, [0, 1]), (gaussian, [1, 0])], [0, 2], "fracs")

    exception_raised = exc_info.value
    assert (
        str(exception_raised)
        == "sum_dists only accepts the parameter position of one fraction."
    )


def test_areas_flag_with_wrong_args():
    with pytest.raises(ValueError) as exc_info:
        sum_dists([(gaussian, [0, 1]), (gaussian, [1, 0])], [0], "areas")

    exception_raised = exc_info.value
    assert str(exception_raised) == "sum_dists needs two parameter indices of areas."
...
def test_onea_area_flag_with_wrong_args():
    ...
    exception_raised = exc_info.value
    assert str(exception_raised) == "sum_dists needs one parameter index of an area."
